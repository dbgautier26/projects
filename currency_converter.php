...
if(isset($_POST['cur1'])&& isset($_POST['cur2'])&& isset($_POST['amount'])) {
 
	//Set our variables
	$currency_given = strtolower($_POST['cur1']);
	$currency_wanted = strtolower($_POST['cur2']);
	$amount = $_POST['amount'];
	...
	if(ctype_digit($amount) && ($amount>0)){
		...
		$random =array_rand($format,1);
			if($format[$random] == 'json'){
				$url='localhost/maviance/ccAPI.php?format=json';
			...
			elseif($format[$random] == 'xml'){
				$url='localhost/maviance/ccAPI.php?format=xml';
